namespace Loupedeck.KgClipstudioPlugin
{
    ...
    public class 編集対象にする下のレイヤー : PluginDynamicCommand
    {
        public 編集対象にする下のレイヤー()
        : base(displayName: "編集対象 にする 下の レイヤー", description: "編集対象 にする 下の レイヤー", groupName: "レイヤーメニュー")
        {
        }
        protected override BitmapImage GetCommandImage(String actionParameter, PluginImageSize imageSize)
        {
...

            {
                if (ResourcePath == null)
                {
                    ...
                    bitmapBuilder.DrawText("編集対象 にする 下の レイヤー", color: new BitmapColor(68,13,43));
