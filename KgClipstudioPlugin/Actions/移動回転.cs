namespace Loupedeck.KgClipstudioPlugin
{
    ...
    public class 移動回転 : PluginDynamicCommand
    {
        public 移動回転()
        : base(displayName: "移動回転", description: "移動回転", groupName: "ツール")
        {
        }
        protected override BitmapImage GetCommandImage(String actionParameter, PluginImageSize imageSize)
        {
...

            {
                if (ResourcePath == null)
                {
                    ...
                    bitmapBuilder.DrawText("移動回転", color: new BitmapColor(241,228,240));
